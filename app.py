...
def upload_image():
    ...
    if operation_selection == 'gray':
        ...
        file_data = rain(decode_array_to_img)
...
def rain(decode_array_to_img):
    raim = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.6)
    status, output_img = cv2.imencode('.PNG', raim)

    return output_img

def invert(decode_array_to_img):
    intel = cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode('.PNG', intel)

    return output_img

def hdr(decode_array_to_img):
    win_11_new_feature = cv2.stylization(decode_array_to_img, sigma_s=12, sigma_r=0.15)
    status, output_img = cv2.imencode('.PNG', win_11_new_feature)
...
print("(c) Copyright Lakshya Singh Chauhan")